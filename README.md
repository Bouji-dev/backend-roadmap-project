...
### Day 4: Alembic in Practice: init → revision → upgrade → downgrade

**Topics**
- Practical database migrations
- Alembic initialization and configuration
- Autogenerating migration files
- Applying and reverting database migrations
- Upgrade and downgrade concepts

**Implementation**
- Initialized Alembic in the project
- Configured Alembic connection to PostgreSQL
- Connected Alembic with SQLAlchemy models
- Created first migration file for users table
- Applied migration using upgrade command

**Key Points**
- Alembic manages database schema changes safely
- Migration files describe incremental database changes
- Autogenerated migrations must be reviewed
- Upgrade applies changes, downgrade reverts them
- Migration files are part of the source code


### Day 5: Database Integration & Repository Pattern
...
- User ORM model
- UserRepository with database queries
- UserService for business logic
- GET /api/v1/users/{user_id} endpoint connected to database

**Key notes:**
- Database user and password must match PostgreSQL credentials
- Repository layer must only handle data access
- Business logic belongs in the service layer
- API layer should stay thin and clean
